...
> 最低系统：**iOS 15.0**（iOS 15.8.8 可正常安装）。
...
- **确认次数（N 次 / T 秒窗口）**：每越过阈值立即记一次冲击，在 T 秒窗口内凑满 N 次才播放；播放后进入冷却清零
- **低频主导判定**：要求低频明显强于中频/高频（说话声会被过滤），并叠加峰值回落与最小事件间隔，避免同一声响重复计数、累积来回跳
- **报警音频自选**：可从“文件”App 选择 mp3 / wav / m4a，也可用 4 种内置合成音（叮咚、蜂鸣、短促滴、钟琴）
- 灵敏度 / 确认次数 / 时间窗口 / 触发冷却 / 麦克风增益实时可调
- **运行状态面板**：低频能量、尖锐度、冲击累积（N/N）、低频占比、综合评分、触发次数、运行时长
- 麦克风输入设备选择（iPhone 由系统决定路由，通常只有内置麦）
